...
var blobBuilder, blob,
    BlobBuilder = this.WebKitBlobBuilder || this.MozBlobBuilder
        || this.BlobBuilder;
...
function nextFile(downloadButton, event) {
  if (g_downResData_Flag) {
    ...
    if (linktype == "link") {
      ...
      global_zipWriter.add(filename, new zip.Data64URIReader(blob),
          function () {
            info[4] = true;
            nextFile(downloadButton, event);
          });
    ...
    for (var key in data) {
      if (typeof key == "string") {
        ...
        for (var i = 0; i < arr.length; i++) {
          ...
          if (key == "img") {
            str += "<li><label><input type='checkbox' data-type='" + key
                + "' data-id='" + key + i + "' data-src='" + arr[i]
                + "' data-link='" + link_type + "' /><img src='" + arr[i]
                + "' /></label></li>";
          } else {
            str += "<li><label><input type='checkbox' data-type='" + key
                + "' data-id='" + key + i + "' data-src='" + arr[i]
                + "' data-link='" + link_type + "' />" + arr[i] + "<a href='"
                + arr[i] + "' target='blank'></a></label></li>";
          }
